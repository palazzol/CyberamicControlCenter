                     ...
                     0055    16 TIMER_100MS_B   = 0x0055    ; decremented every 0.1s
                     0056    17 TIMER_100MS_R   = 0x0056    ; decremented every 0.1s, resets to 100
                     0057    18 TIMER_10S       = 0x0057    ; decremented every 10s
                     ...
                     005B    22 PROG_STATE      = 0x005B    ; 0x00=PROG was not pushed, 0x80=Prog was pushed
   ...
   1C4F 85 53         [ 3]   71         sta     TIMER_1MS_R                             ; 100 - init 1 msec master counter
   1C51 A9 2B         [ 2]   72         lda     #0x2B
   1C53 85 57         [ 3]   73         sta     TIMER_10S                               ; ~5 minute timer?
   1C55 A9 10         [ 2]   74         lda     #0x10                                   ; 16
   1C57 85 63         [ 3]   75         sta     AGC_GAIN                                ; Set initial AGC gain value
   ...
   1C60 85 54         [ 3]   79         sta     TIMER_100MS_A
   1C62                      80 $1:
   1C62 20 13 1E      [ 6]   81         jsr     TUPDATE                                 ; do not much for 4 seconds
   1C65 A5 54         [ 3]   82         lda     TIMER_100MS_A
   ...
   1C71 A5 58         [ 3]   89         lda     ZEROCROSS_CTR
   ...
   1CAB A5 5A         [ 3]  115         lda     PROG_CTR                                ; wait until we are triggered
   ...
   1CB9 A5 57         [ 3]  121         lda     TIMER_10S                               ; has the ~5 minute timer run out?
   1CBB D0 E8         [ 4]  122         bne     WAITPLAY                                ; no, keep looping
   1CBD E6 5A         [ 5]  123         inc     PROG_CTR                                ; yes, simulate a PROG button press
   ...
   1CD4 C6 5A         [ 5]  134         dec     PROG_CTR                                ; no longer triggered
   1CD6 20 87 1D      [ 6]  135         jsr     PLAYTRK                                 ; play a track!
   1CD9 20 F9 1C      [ 6]  136         jsr     INITBRDS                                ; init the boards
   1CDC A9 2B         [ 2]  137         lda     #0x2B
   1CDE 85 57         [ 3]  138         sta     TIMER_10S                               ; set a ~5 minute timer
   1CE0 E6 59         [ 5]  139         inc     TRACK_CTR                               ; track counter
   1CE2 A5 59         [ 3]  140         lda     TRACK_CTR
   ...
   1D24 85 50         [ 3]  182         sta     TIMER_1MS_A
   1D26                     183 $6:
   1D26 20 13 1E      [ 6]  184         jsr     TUPDATE                                 ; check for PROG button push
   1D29 A5 50         [ 3]  185         lda     TIMER_1MS_A
   ...
   1D35 85 58         [ 3]  196         sta     ZEROCROSS_CTR
   ...
   1D41 85 54         [ 3]  203         sta     TIMER_100MS_A                           ; ~.4 secs?
   1D43                     204 $29:
   ...
   1D4B E6 58         [ 5]  208         inc     ZEROCROSS_CTR                           ; count edges
   1D4D AD 02 03      [ 4]  209         lda     transport_periph$ddr_reg_b
   1D50 A5 58         [ 3]  210         lda     ZEROCROSS_CTR
   1D52 C9 60         [ 2]  211         cmp     #0x60                                   ; 96 edges?
   1D54 B0 0A         [ 4]  212         bcs     $10                                     ; exit
   1D56                     213 $28:
   1D56 A5 54         [ 3]  214         lda     TIMER_100MS_A                           ; ~.4 secs?
   1D58 D0 E9         [ 4]  215         bne     $29
   1D5A A5 58         [ 3]  216         lda     ZEROCROSS_CTR
   ...
   1D63 85 50         [ 3]  228         sta     TIMER_1MS_A                             ; 250 msec
   1D65                     229 $11:
   1D65 20 13 1E      [ 6]  230         jsr     TUPDATE                                 ; housekeeping
   1D68 A5 50         [ 3]  231         lda     TIMER_1MS_A
   ...
   1D82 A5 50         [ 3]  247         lda     TIMER_1MS_A
   ...
   1DBE 85 5C         [ 3]  285         sta     TAPE_BYTE                               ; store it here
   ...
   1DCA A5 5C         [ 3]  291         lda     TAPE_BYTE                               ; get original byte
   1DCC 38            [ 2]  292         sec
   ...
   1DE7 85 5D         [ 3]  307         sta     SOL_MASK                                ; store mask in SOL_MASK
   1DE9 A5 5E         [ 3]  308         lda     CURR_CHANNEL
   1DEB 4A            [ 2]  309         lsr     a                                       ; get on/off in carry
   1DEC B0 09         [ 4]  310         bcs     $17                                     ; if on, jump
   1DEE A5 5D         [ 3]  311         lda     SOL_MASK
   ...
   1DF6 60            [ 6]  315         rts
                            316 ;
   1DF7                     317 $17:
   1DF7 A5 5D         [ 3]  318         lda     SOL_MASK
   1DF9 15 00         [ 4]  319         ora     RAM_start,x
   1DFB 95 00         [ 4]  320         sta     RAM_start,x                             ; turn on solenoid
   1DFD 60            [ 6]  321         rts
                            322 ;
   1DFE                     323 PROCCHNL:
   1DFE A5 5C         [ 3]  324         lda     TAPE_BYTE
   1E00 85 5E         [ 3]  325         sta     CURR_CHANNEL
   1E02                     326 $18:
   1E02 60            [ 6]  327         rts
                            328 ;
                            329 ; bit mask table
   ...
   1E16 85 5F         [ 3]  339         sta     TIMER_TMP                               ; store this state in 5F
   1E18 F0 44         [ 4]  340         beq     TEXIT                                   ; No flags set, return
   1E1A A5 5B         [ 3]  341         lda     PROG_STATE                              ; Are we already running?
   1E1C 30 0E         [ 4]  342         bmi     $19                                     ; yes, jump ahead
   1E1E A5 5F         [ 3]  343         lda     TIMER_TMP                               ; else check flags
   1E20 29 40         [ 2]  344         and     #0x40                                   ; PROG pushed?
   1E22 F0 16         [ 4]  345         beq     RDTIMER                                 ; if not, go to read timer
   1E24 A9 80         [ 2]  346         lda     #0x80
   1E26 85 5B         [ 3]  347         sta     PROG_STATE                              ; PROG Button pushed
   1E28 A9 FA         [ 2]  348         lda     #0xFA
   1E2A 85 51         [ 3]  349         sta     TIMER_1MS_B
   1E2C                     350 $19:
   1E2C A5 51         [ 3]  351         lda     TIMER_1MS_B                             ; for 250ms?
   1E2E D0 06         [ 4]  352         bne     $20                                     ; no, exit
   1E30 A9 00         [ 2]  353         lda     #0x00
   1E32 85 5B         [ 3]  354         sta     PROG_STATE                              ; yes, reset PROG button state
   1E34 E6 5A         [ 5]  355         inc     PROG_CTR                                ; and mark as running
   1E36                     356 $20:
   1E36 A5 5F         [ 3]  357         lda     TIMER_TMP                               ; check timer irq bit
   ...
   1E42 C6 50         [ 5]  363         dec     TIMER_1MS_A                             ; decrement these timers every timer reset (1ms)
   1E44 C6 51         [ 5]  364         dec     TIMER_1MS_B
   1E46 C6 52         [ 5]  365         dec     TIMER_1MS_C
   1E48 C6 53         [ 5]  366         dec     TIMER_1MS_R
   1E4A D0 12         [ 4]  367         bne     TEXIT                                   ; if timer TIMER_1MS_R expires, then wrap to 100
   1E4C A9 64         [ 2]  368         lda     #0x64                                   ; 100
   1E4E 85 53         [ 3]  369         sta     TIMER_1MS_R
   1E50 C6 54         [ 5]  370         dec     TIMER_100MS_A
   1E52 C6 55         [ 5]  371         dec     TIMER_100MS_B
   1E54 C6 56         [ 5]  372         dec     TIMER_100MS_R
   1E56 D0 06         [ 4]  373         bne     TEXIT                                   ; if timer TIMER_100MS_R expires, then wrap to 100
   1E58 A9 64         [ 2]  374         lda     #0x64                                   ; 100
   1E5A 85 56         [ 3]  375         sta     TIMER_100MS_R
   1E5C C6 57         [ 5]  376         dec     TIMER_10S
   1E5E                     377 TEXIT:
   1E5E 60            [ 6]  378         rts
                            379 ;
                            380 ;       Read the AGC mic level
                            381 ;       Take the average of 8 samples, and put it into AGC_LEVEL (range is 0 to 8)
                            382 ;
   1E5F                     383 AGCMICRD:
   1E5F A9 00         [ 2]  384         lda     #0x00
   1E61 85 61         [ 3]  385         sta     AGC_ACCUM                               ; init final agc value
   1E63 85 62         [ 3]  386         sta     AGC_SAMPLES                             ; init agc sample counter
   1E65 A9 0A         [ 2]  387         lda     #0x0A
   1E67 85 54         [ 3]  388         sta     TIMER_100MS_A                           ; Start a 1 second timer
   1E69                     389 $23:
   1E69 20 13 1E      [ 6]  390         jsr     TUPDATE                                 ; housekeeping
   1E6C A5 54         [ 3]  391         lda     TIMER_100MS_A
   1E6E D0 F9         [ 4]  392         bne     $23                                     ; if 1 sec, do housekeeping
   1E70 A9 0A         [ 2]  393         lda     #0x0A
   1E72 85 54         [ 3]  394         sta     TIMER_100MS_A                           ; reset timer
   1E74 A5 62         [ 3]  395         lda     AGC_SAMPLES
   1E76 C9 08         [ 2]  396         cmp     #0x08                                   ; 8 samples?
   1E78 F0 15         [ 4]  397         beq     $27                                     ; yes - jump to final calculation
   1E7A E6 62         [ 5]  398         inc     AGC_SAMPLES                             ; increment the sample counter
   ...
   1E88 65 61         [ 3]  408         adc     AGC_ACCUM                               ; add it into AGC_ACCUM (do this 8 times)
   1E8A 85 61         [ 3]  409         sta     AGC_ACCUM
   1E8C 4C 69 1E      [ 3]  410         jmp     $23
                            411 ;
   1E8F                     412 $27:
   1E8F 46 61         [ 5]  413         lsr     AGC_ACCUM                               ; divide by 8 (average of 8 samples)
   1E91 46 61         [ 5]  414         lsr     AGC_ACCUM
   1E93 46 61         [ 5]  415         lsr     AGC_ACCUM
   1E95 A5 61         [ 3]  416         lda     AGC_ACCUM
   1E97 85 60         [ 3]  417         sta     AGC_LEVEL                               ; store agc value in AGC_LEVEL
   1E99 A9 00         [ 2]  418         lda     #0x00
   1E9B 85 61         [ 3]  419         sta     AGC_ACCUM                               ; clear these 2 and return
   1E9D 85 62         [ 3]  420         sta     AGC_SAMPLES
   1E9F 60            [ 6]  421         rts
                            422 ;
   ...
   1EA8 4A            [ 2]  431         lsr     a
   1EA9 18            [ 2]  432         clc
   1EAA 65 60         [ 3]  433         adc     AGC_LEVEL                               ; add audio level to it
   1EAC AA            [ 2]  434         tax
   1EAD BD D3 1E      [ 5]  435         lda     AGCTABLE,x                              ; and get the table value
   1EB0 85 63         [ 3]  436         sta     AGC_GAIN                                ; store this value in AGC_GAIN
   1EB2 A5 52         [ 3]  437         lda     TIMER_1MS_C                             ; 10ms timer expired?
   1EB4 D0 16         [ 4]  438         bne     $26                                     ; no, just update CPU Leds
   1EB6 A9 0A         [ 2]  439         lda     #0x0A
   1EB8 85 52         [ 3]  440         sta     TIMER_1MS_C                             ; restart 10ms timer
   1EBA A5 63         [ 3]  441         lda     AGC_GAIN                                ; every 10ms, adjust gain by 1 if needed
